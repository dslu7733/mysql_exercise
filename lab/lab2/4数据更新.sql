...
begin
  ...
  #与病人同一车次同一车厢的乘客
  declare contactorCur cursor for select seatRow, seatNo, pCardID, sstationID, astationID 
    from taketrainrecord where tid = in_tID and carrigeId = in_carrigeID and sstatus = 1;
...
# 辅助视图，获取病人相关信息
create view patientView(pCardID, tid, ADate, carrigeID, seatRow, seatNo, AStationID, SStationID ) as 	
  select t.pCardID, t.tid, date(trainpass.ADatetime) as ADate, CarrigeID, SeatRow, SeatNo, t.AStationID, t.SStationID 
    from diagnoserecord as d join taketrainrecord as t on (DStatus = 1 and SStatus = 1 and d.PCardID = t.PCardID) 
...
begin
  ...
  while @tripNum > 0 do
    ...
    #获取病人该次乘车的信息
    select ADate, pCardID, tid, carrigeID, seatRow, seatNo, AStationID, SStationID  	
