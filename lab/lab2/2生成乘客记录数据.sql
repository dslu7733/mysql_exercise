...
begin
    ...
    repeat	#开始遍历
	  if (@preTID != trainID) then  	#同一列车流水
        repeat 		# 2个车厢
          ...
          set sNoMax = ( select count(*) from TrainPass where TID = trainID );	#这趟车经过的站点数目
            ...
            repeat					#一个车厢的乘客	
			  # 从第一个开始按顺序抽取乘客，获得他的身份证号
              if @passengerNum <= passengerTol then		# 这里PID是连续的前提是乘客表没有删减过元组
              ...
              set @tolNum :=  @tolNum - 1;
			until @tolNum = 0 end repeat;		#一个车厢的乘客数目清零，结束循环
                
            set @carrageID := @carrageID + 1;
		until @carrageID > 2 end repeat;	# 车厢数目大于2，结束循环
      ...
      fetch trainPassCur into trainID, stationID;
      if( @passengerNum > 900 ) then 		# 考虑到后面实验的需要，当乘客超过900人特意添加武汉出发的乘客
        repeat
          fetch trainPassCur into trainID, stationID;
	    until (@passengerNum > 1000 or stationID = @wuhanSID) end repeat;
      end if;
      
    until @passengerNum > 1000 end repeat;		# 遍历结束, 生成不少于1000个乘客
    close trainPassCur;
end //
delimiter ;
